	...
	ColumnLayout {
        ...
        Canvas {
            ...
            onPaint: {
                ...
                if (!Julia.get_game_started() ) {
                    // add game over
                    if (Julia.get_game_over()) {
                        ctx.fillStyle = "white"
                ...
                ctx.fillText(lines < 200 ? Math.floor(lines / 10)+1 : 20, 280, 240)
                // add score
                ctx.fillText("Score", 280, 280)
                ctx.clearRect(280, 280, 100, 20)
                ctx.fillText(Julia.get_score(), 280, 300)
                ctx.stroke()
                // add help
